           ...
           /* if(!jPlaceDetails.isNull("international_phone_number")){
            ...
            if (!jPlaceDetails.isNull("opening_hours")) {
                try {
                    ...
                    JSONArray jsonArray = jPlaceDetails.getJSONObject("opening_hours").getJSONArray("weekday_text");

                    String temp = "day";
                    for (int i = 0; i < jsonArray.length(); i++) {
                        hPlaceDetails.put(temp + i, jsonArray.get(i).toString());
                        //  Log.d(temp+i,jsonArray.get(i).toString());
                    }
                } catch (JSONException e) {
            ...
            if (!jPlaceDetails.isNull("reviews")) {

                try {

                    hPlaceDetails.put("review", jPlaceDetails.getJSONArray("reviews").toString());
